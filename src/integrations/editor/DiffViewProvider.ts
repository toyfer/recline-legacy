...
export class DiffViewProvider {
    ...
    private currentDiffId?: string; // Track current diff operation to optimize search
    ...
    async open(relPath: string): Promise<void> {
        ...
        // Generate a new unique ID for this diff operation
        this.currentDiffId = Date.now().toString();
    }

    async update(accumulatedContent: string, isFinal: boolean) {
	// Pass the current diffId to track diffs across chunks
	const diffId = this.currentDiffId;
        if (!this.relPath || !this.activeLineController || !this.fadedOverlayController) {
            throw new Error("Required values not set");
        }

        // If this is a new diff operation, generate a new ID
        if (!this.isEditing || !this.currentDiffId) {
            this.currentDiffId = Date.now().toString();
        }
        ...
        try {
            // Process the content with diffId for optimized searching first
            const processedContent = await constructNewFileContent(accumulatedContent, this.originalContent ?? "", isFinal, diffId);
            this.newContent = processedContent;

            // Update the view with the processed content
            const edit = new vscode.WorkspaceEdit();
            const rangeToReplace = new vscode.Range(0, 0, document.lineCount, 0);
            edit.replace(document.uri, rangeToReplace, processedContent);
            await vscode.workspace.applyEdit(edit);

            // Update the streamedLines with the processed content
            this.streamedLines = processedContent.split("\n");

            if (isFinal) {
                // Add empty last line if original content had one
                const hasEmptyLastLine = this.originalContent?.endsWith("\n");
                if (hasEmptyLastLine && !processedContent.endsWith("\n")) {
                    this.newContent += "\n";
                    const finalEdit = new vscode.WorkspaceEdit();
                    finalEdit.insert(document.uri, new vscode.Position(document.lineCount, 0), "\n");
                ...
                // Reset the diff operation ID since this operation is complete
                this.currentDiffId = undefined;
            }
        } catch (error) {
            // If there's an error processing the diff, revert to original content
            const revertEdit = new vscode.WorkspaceEdit();
            revertEdit.replace(document.uri, new vscode.Range(0, 0, document.lineCount, 0), this.originalContent ?? "");
    ...
    async reset() {
        ...
        this.streamedLines = [];
        this.preDiagnostics = [];
        this.currentDiffId = undefined; // Reset diff operation ID
    }
}
