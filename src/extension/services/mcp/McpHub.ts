...
export class McpHub {
  ...
  private appendErrorMessage(connection: McpConnection, error: string): void {
    const newError: string = (connection.server.error != null && connection.server.error.length > 0)
  ...
  private async fetchResourcesList(serverName: string): Promise<McpResource[]> {
    try {
      const response = await this.connections
        ...
        ?.client
        .request({ method: "resources/list" }, ListResourcesResultSchema);
  ...
  private async fetchResourceTemplatesList(serverName: string): Promise<McpResourceTemplate[]> {
    try {
      const response = await this.connections
        .find(conn => conn.server.name === serverName)
        ?.client
        .request({ method: "resources/templates/list" }, ListResourceTemplatesResultSchema);
  ...
  private async fetchToolsList(serverName: string): Promise<McpTool[]> {
    try {
      const response = await this.connections
        .find(conn => conn.server.name === serverName)
        ?.client
        .request({ method: "tools/list" }, ListToolsResultSchema);
  ...
  private removeAllFileWatchers() {
    this.fileWatchers.forEach(async watcher => watcher.unsubscribe());
    this.fileWatchers.clear();
  }

  private setupFileWatcher(name: string, config: StdioServerParameters) {
  ...
  async updateServerConnections(newServers: Record<string, any>): Promise<void> {
    ...
    for (const [name, config] of Object.entries(newServers)) {
      ...
      else if (!isEqual(JSON.parse(currentConnection.server.config), config)) {
