...
import type { ApiConfiguration, MessageParamWithTokenCount } from "@shared/api";
import type {
  BrowserAction,
  BrowserActionResult,
  ReclineApiReqCancelReason,
  ReclineApiReqInfo,
  ReclineAsk,
...
export class Recline {
  ...
  private async resumeTaskFromHistory() {
    ...
    // Now present the recline messages to the user and ask if they want to resume

    const lastReclineMessage = this.reclineMessages
      .slice()
      .reverse()
    ...
    if (lastReclineMessage?.ask === "completion_result") {
    ...
    // need to make sure that the api conversation history can be resumed by the api, even if it goes out of sync with recline messages

    let existingApiConversationHistory: MessageParamWithTokenCount[]
    ...
    let modifiedApiConversationHistory: MessageParamWithTokenCount[]; // need to remove the last user message to replace with new modified user message
    if (existingApiConversationHistory.length > 0) {
      ...
      if (lastMessage.role === "assistant") {
        ...
        const previousAssistantMessage: MessageParamWithTokenCount | undefined
    ...
    const agoText = (() => {
      const timestamp = lastReclineMessage?.ts ?? Date.now();
      const now = Date.now();
    ...
    switch (block.type) {
      case "text": {
        ...
        if (content) {
          ...
          if (lastOpenBracketIndex !== -1) {
            ...
            if (!hasCloseBracket) {
              ...
              const isLikelyTagName = /^[a-z_]+$/i.test(tagContent);
        ...
        if (!block.partial) {
          ...
          const match = content?.trimEnd().match(/```[\w-]+$/);
          if (match) {
            ...
            try {
              if (block.partial) {
                ...
                if (this.shouldAutoApproveTool(block.name)) {
                  ...
                  showNotificationForApprovalIfAutoApprovalEnabled(
                    `Recline wants to view directory ${path.basename(absolutePath)}/`
                  );
                  await this.removeLastPartialMessageIfExistsWithType("say", "tool");
                  const didApprove = await askApproval("tool", completeMessage);
                  if (!didApprove) {
            ...
            try {
              if (block.partial) {
                ...
                if (!requiresApproval && this.shouldAutoApproveTool(block.name)) {
                  ...
                  showNotificationForApprovalIfAutoApprovalEnabled(
                    `Recline wants to execute a command: ${command}`
                  );
              ...
              if (block.partial) {
                ...
                if (mcp_arguments) {
                  try {
                    ...
                    await this.say(
                      "error",
                      `Recline tried to use ${tool_name} with an invalid JSON argument. Retrying...`
                    );
                    pushToolResult(
                      formatResponse.toolError(
  ...
  async recursivelyMakeReclineRequests(
    ...
    if (
      ...
      await this.ask(
        ...
        `Recline has auto-approved ${this.autoApprovalSettings.maxRequests.toString()} API requests. Would you like to reset the count and proceed with the task?`
      );
    ...
    const previousApiReqIndex = findLastIndex(this.reclineMessages, m => m.say === "api_req_started");
    ...
    const lastApiReqIndex = findLastIndex(this.reclineMessages, m => m.say === "api_req_started");
      ...
      try {
        for await (const chunk of stream) {
          ...
          if (this.abort) {
            ...
            if (!this.abandoned) {
              // only need to gracefully abort if this instance isn't abandoned (sometimes openrouter stream hangs, in which case this would affect future instances of recline)
        ...
        if (!this.abandoned) {
          ...
          if (history) {
            await this.providerRef.deref()?.initReclineWithHistoryItem(history.historyItem);
            // await this.providerRef.deref()?.postStateToWebview()
          }
        }
      }
