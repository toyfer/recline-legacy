...
function HistoryView({ onDone }: HistoryViewProps) {
  ...
  const handleDeleteHistoryItem = (id: string) => {
    vscodeApiWrapper.postMessage({ type: "deleteTaskWithId", text: id });
  };
...
function ExportButton({ itemId }: { itemId: string }) {
  return (
    <VSCodeButton
      ...
      onClick={(e) => {
        e.stopPropagation();
        vscodeApiWrapper.postMessage({ type: "exportTaskWithId", text: itemId });
      }}
    >
