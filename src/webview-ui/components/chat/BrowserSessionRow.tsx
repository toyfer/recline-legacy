  ...
  const pages = useMemo(() => {
    const result: {
      currentState: {
        ...
        consoleLogs?: string;
        messages: ReclineMessage[]; // messages up to and including the result
      };
      nextAction?: {
        messages: ReclineMessage[]; // messages leading to next result
      };
    }[] = [];

    let currentStateMessages: ReclineMessage[] = [];
    let nextActionMessages: ReclineMessage[] = [];
    ...
    <div style={{ padding: "10px 6px 10px 15px", marginBottom: -10 }}>
      <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "10px" }}>
        ...
        <span style={{ fontWeight: "bold" }}>
          <>{isAutoApproved ? "Recline is using the browser:" : "Recline wants to use the browser:"}</>
        </span>
      </div>
      <div
        style={{
        ...
        >
          ...
          {displayState.mousePosition && (
            <BrowserCursor
              style={{
                position: "absolute",
                top: `${(Number.parseInt(mousePosition.split(",")[1]) / 600) * 100}%`,
                left: `${(Number.parseInt(mousePosition.split(",")[0]) / 900) * 100}%`,
...
function BrowserSessionRowContent({
  message,
  isExpanded,
  onToggleExpand,
  lastModifiedMessage,
