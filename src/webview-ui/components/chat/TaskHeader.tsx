...
export function highlightMentions(text?: string, withShadow = true) {
  if (!text)
    return text;
    ...
    if (index % 2 === 0) {
      ...
      return (
        <span
          ...
          onClick={() => vscodeApiWrapper.postMessage({ type: "openMention", text: part })}
        >
          @
          {part}
        </span>
...
function ExportButton() {
  return (
    <VSCodeButton
      appearance="icon"
      onClick={() => vscodeApiWrapper.postMessage({ type: "exportCurrentTask" })}
      style={
        {
          // marginBottom: "-2px",
          // marginRight: "-2.5px",
