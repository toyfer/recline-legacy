...
function ApiOptions({ showModelOptions, apiErrorMessage, modelIdErrorMessage }: ApiOptionsProps) {
  ...
  // Poll ollama/lmstudio/vscode-lm models
  const requestLocalModels = useCallback(() => {
    if (selectedProvider === "ollama") {
      vscodeApiWrapper.postMessage({ type: "requestOllamaModels", text: apiConfiguration?.ollamaBaseUrl });
    }
    else if (selectedProvider === "lmstudio") {
      vscodeApiWrapper.postMessage({ type: "requestLmStudioModels", text: apiConfiguration?.lmStudioBaseUrl });
    }
    else if (selectedProvider === "vscode-lm") {
      vscodeApiWrapper.postMessage({ type: "requestVsCodeLmSelectors" });
    }
  ...
  const handleMessage = useCallback((event: MessageEvent) => {
    ...
    else if (message.type === "vsCodeLmSelectors" && message.vsCodeLmSelectors) {
      setVsCodeLmSelectors(message.vsCodeLmSelectors);
    }
  }, []);
        ...
        <div>
          ...
          >
            <span style={{ color: "var(--vscode-errorForeground)" }}>
              ...
              {" "}
              Recline uses complex prompts and works best
                  ...
                  onChange={(e) => {
                    ...
                    if (value) {
                      handleInputChange("vsCodeLmModelSelector")({
                        target: { value: parseVsCodeLmModelSelector(value) }
                      });
                    }
                  }}
                  style={{ width: "100%" }}
...
export function getOpenRouterAuthUrl(uriScheme?: string) {
  return `https://openrouter.ai/auth?callback_url=${uriScheme || "vscode"}://julesmons.recline/openrouter`;
}
...
function ModelInfoSupportsItem({
  isSupported,
  supportsLabel,
  doesNotSupportLabel
}: {
